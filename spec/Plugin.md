...
| `version` | string | **必須** | プラグインのバージョン。`major.minor.runtime` 形式（例: `1.2.0`）。runtime は `0=JavaScript`, `1=PHP`。 |
| `minAppVersion` | string | **必須** | 対応するアプリ最小バージョン。未指定は非互換扱い。現在は `1.0.0` 固定。 |
...
| `api` | object | 任意 | 利用する外部API情報。`name` は必須、`baseUrl` は任意（指定時はURL文字列）。 |
